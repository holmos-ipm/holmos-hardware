...
if __name__ == '__main__':
    ...
    scad_render_to_file(round_mount_light(20, opening_angle=None, stop_inner_diam=19),
                        "scad/misc/objective_mount_edmund4x_simple.scad", file_header=header)

    scad_render_to_file(round_mount_light(24, opening_angle=None, stop_inner_diam=21),
                        "scad/misc/objective_mount_edmund4x_plan.scad", file_header=header)

    scad_render_to_file(round_mount_light(20, opening_angle=0, cyl_length=40, ring_thick=2), "scad/misc/light_tube.scad", file_header=header)

    # with stop - lenses
    for d in (25.4, 20, 16.5):
        scad_render_to_file(round_mount_light(d, opening_angle=None, stop_inner_diam=d-2),
                        "scad/misc/lens mount_d{:.1f}.scad".format(d), file_header=header)

    # without stop - lasers
    for d in (12, 10):
        scad_render_to_file(round_mount_light(d, opening_angle=None), "scad/misc/round_mount_d{:.1f}.scad".format(d), file_header=header)
